 ...
 * @param camarones - Arreglo de enteros representando los camarones
 *   - Números negativos: camarones naranjas
 *   - Ceros: camarones azules  
 *   - Números positivos: camarones verdes
 * @returns Arreglo con [peso_naranja, peso_azul, peso_verde] con 6 decimales
 */
export function shrimpWeight(camarones: number[]): number[] {
    ...
    for (const camaron of camarones) {
        if (camaron < 0) {
            naranjas++;
        } else if (camaron === 0) {
            azules++;
 ...
 */
export function testShrimpWeight(): void {
